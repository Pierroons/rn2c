...
public class UserMenu implements PermissionsPopupMenu.IOnMenuPrepareListener, PopupMenu.OnMenuItemClickListener {
    ...
    public void onMenuPrepare(Menu menu, int permissions) {
        ...
        // "Définir le rang" visible si : user enregistré, pas soi-même, et on a la permission Register ou Write
        menu.findItem(R.id.context_set_rank).setVisible(!self && mUser.getUserId() >= 0 &&
                (perms & (Permissions.Register | Permissions.Write)) > 0);
        // "Réinitialiser le certificat" visible si : user enregistré, pas soi-même, et on a la permission Register ou Write
        menu.findItem(R.id.context_reset_cert).setVisible(!self && mUser.getUserId() >= 0 &&
                (perms & (Permissions.Register | Permissions.Write)) > 0);
...
//        } else if (itemId == R.id.context_info) {
        ...
        } else if (itemId == R.id.context_set_rank) {
            showRankDialog();
        } else if (itemId == R.id.context_reset_cert) {
            new MaterialAlertDialogBuilder(mContext)
                    .setTitle(R.string.rn2c_reset_cert)
                    .setMessage(mContext.getString(R.string.rn2c_reset_cert_confirm, mUser.getName()))
                    .setPositiveButton(R.string.confirm, (dialog, which) -> {
                        mService.kickBanUser(mUser.getSession(), mContext.getString(R.string.rn2c_reset_cert_reason), false);
                    })
                    .setNegativeButton(android.R.string.cancel, null)
                    .show();
        } else {
    ...
    private void showRankDialog() {
        int perms = mService.getPermissions();
        boolean isAdmin = (perms & Permissions.Write) > 0;

        // Moderator peut donner user ou moderator, admin peut aussi donner admin
        final String[] rankNames;
        final String[] rankGroups;
        if (isAdmin) {
            rankNames = new String[]{mContext.getString(R.string.rn2c_rank_user),
                    mContext.getString(R.string.rn2c_rank_moderator),
                    mContext.getString(R.string.rn2c_rank_admin)};
            rankGroups = new String[]{"", "moderator", "admin"};
        } else {
            rankNames = new String[]{mContext.getString(R.string.rn2c_rank_user),
                    mContext.getString(R.string.rn2c_rank_moderator)};
            rankGroups = new String[]{"", "moderator"};
        }

        new MaterialAlertDialogBuilder(mContext)
                .setTitle(mContext.getString(R.string.rn2c_rank_title, mUser.getName()))
                .setItems(rankNames, (dialog, which) -> {
                    String group = rankGroups[which];
                    if (group.isEmpty()) {
                        // Retirer de tous les groupes privilégiés
                        mService.setUserGroup(mUser.getUserId(), "");
                    } else {
                        mService.setUserGroup(mUser.getUserId(), group);
                    }
                })
