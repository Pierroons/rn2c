...
public class HumlaService extends Service implements IHumlaService, IHumlaSession, HumlaConnection.HumlaConnectionListener, HumlaLogger, BluetoothScoReceiver.Listener {
    ...
    public static final String EXTRAS_VOICE_CHANGER = "voice_changer";
    /** If true, accept all server certificates without verification (for .onion servers). */
    public static final String EXTRAS_TRUST_ALL_CERTIFICATES = "trust_all_certificates";
    ...
    protected void connect() {
        try {
            ...
            mConnection.setTrustAllServerCertificates(mTrustAllCertificates);
            ...
            // Handler pour les réponses ACL (gestion des rangs)
            mConnection.addTCPMessageHandlers(new HumlaTCPMessageListener.Stub() {
                @Override
                public void messageACL(Mumble.ACL msg) {
                    handleACLResponse(msg);
                }
            });
...
//            auth.addCeltVersions(Constants.CELT_11_VERSION);
    ...
    public boolean configureExtras(Bundle extras) throws AudioException {
        ...
        if (extras.containsKey(EXTRAS_TRUST_ALL_CERTIFICATES)) {
            mTrustAllCertificates = extras.getBoolean(EXTRAS_TRUST_ALL_CERTIFICATES);
            reconnectNeeded = true;
        }
        ...
        if (extras.containsKey(EXTRAS_VOICE_CHANGER)) {
            mAudioBuilder.setVoiceChangerEnabled(extras.getBoolean(EXTRAS_VOICE_CHANGER));
        }
    ...
    // Pending group modification
    private int mPendingGroupUserId = -1;
    private String mPendingGroupName = null;

    @Override
    public void setUserGroup(int userId, String groupName) {
        mPendingGroupUserId = userId;
        mPendingGroupName = groupName;
        // Query ACL for root channel (id=0)
        Mumble.ACL.Builder ab = Mumble.ACL.newBuilder();
        ab.setChannelId(0);
        ab.setQuery(true);
        getConnection().sendTCPMessage(ab.build(), HumlaTCPMessageType.ACL);
    }

    // Called when ACL response arrives
    public void handleACLResponse(Mumble.ACL msg) {
        if (mPendingGroupUserId < 0 || mPendingGroupName == null) return;

        int userId = mPendingGroupUserId;
        String targetGroup = mPendingGroupName;
        mPendingGroupUserId = -1;
        mPendingGroupName = null;

        // Rebuild ACL with modified groups
        Mumble.ACL.Builder ab = Mumble.ACL.newBuilder();
        ab.setChannelId(0);

        // Copy existing groups, modifying membership
        String[] privilegeGroups = {"admin", "moderator"};
        for (Mumble.ACL.ChanGroup group : msg.getGroupsList()) {
            Mumble.ACL.ChanGroup.Builder gb = Mumble.ACL.ChanGroup.newBuilder(group);
            boolean isPrivilegeGroup = false;
            for (String g : privilegeGroups) {
                if (group.getName().equals(g)) {
                    isPrivilegeGroup = true;
                    break;
                }
            }
            if (isPrivilegeGroup) {
            ...
            ab.addGroups(gb.build());
        }

        // Copy existing ACLs unchanged
        ab.addAllAcls(msg.getAclsList());

        getConnection().sendTCPMessage(ab.build(), HumlaTCPMessageType.ACL);
        Log.i(TAG, "User " + userId + " ajouté au groupe " + targetGroup);
    }
