...
public class AudioHandler extends HumlaNetworkListener implements AudioInput.AudioInputListener {
    ...
    private final boolean mVoiceChangerEnabled;
    private final float mPitchShiftFactor; // <1 = grave, >1 = aigu
    ...
    public AudioHandler(Context context, HumlaLogger logger, int audioStream, int audioSource,
                        ...
                        boolean preprocessorEnabled, boolean voiceChangerEnabled,
        ...
        mVoiceChangerEnabled = voiceChangerEnabled;
        mPitchShiftFactor = 1.4f; // ~+6 demi-tons (voix méconnaissable)
    ...
    public synchronized void initialize(User self, int maxBandwidth, HumlaUDPMessageType codec) throws AudioException {
        ...
        // Lier AudioTrack à la session AudioRecord pour l'AEC
        int aecSessionId = mInput.getAudioSessionId();
        mOutput.startPlaying(mBluetoothOn ? AudioManager.STREAM_VOICE_CALL : mAudioStream, aecSessionId);
    ...
    public void onAudioInputReceived(short[] frame, int frameSize) {
        ...
        if (talking) {
            // Pitch shift pour modifier la voix (anonymat vocal)
            if (mVoiceChangerEnabled) {
                short[] shifted = new short[frameSize];
                for (int i = 0; i < frameSize; i++) {
                    float srcIdx = i / mPitchShiftFactor;
                    int idx0 = (int) srcIdx;
                    float frac = srcIdx - idx0;
                    if (idx0 + 1 < frameSize) {
                        shifted[i] = (short) (frame[idx0] * (1.0f - frac) + frame[idx0 + 1] * frac);
                    } else if (idx0 < frameSize) {
                        shifted[i] = frame[idx0];
                    } else {
                        shifted[i] = 0;
                    }
                }
                System.arraycopy(shifted, 0, frame, 0, frameSize);
            }
    ...
    public static class Builder {
        ...
        private boolean mPreprocessorEnabled;
        private boolean mVoiceChangerEnabled;
        ...
        public Builder setVoiceChangerEnabled(boolean voiceChangerEnabled) {
            mVoiceChangerEnabled = voiceChangerEnabled;
            return this;
        }
        ...
        public AudioHandler initialize(User self, int maxBandwidth, HumlaUDPMessageType codec, byte targetId) throws AudioException {
            AudioHandler handler = new AudioHandler(mContext, mLogger, mAudioStream, mAudioSource,
                    ...
                    mPreprocessorEnabled, mVoiceChangerEnabled, mEncodeListener, mTalkingListener);
